...
public class Conexao {
		...
		try {
			...
			if (tDriver == null || tDriver.isEmpty())
				throw new InvalidPropertiesFormatException(
						"Propriedade 'driver' não existe ou em branco no arquivo 'jdbc.properties'");
			String tURL = tPropriedades.getProperty("url");
			if (tURL == null || tURL.isEmpty())
				throw new InvalidPropertiesFormatException(
						"Propriedade 'url' não existe ou em branco no arquivo 'jdbc.properties'");
			...
			ExceptionUtil.mostrarErro(tExcept, "Classe do Driver não encontrado");
			System.exit(9);
	...
	public static void desconectar() {
		try {
			// Desconectando do banco de dados e verificando se a desconex~]ao
			// foi bem realizada
	...
	public static void iniciarTransacao() {
		try {
			...
			ExceptionUtil.mostrarErro(tExcept, "Problemas no início da tansação com o banco de dados");
			System.exit(9);
		}
	}
	...
	public static void confirmarTransacao() {
		try {
			...
			ExceptionUtil.mostrarErro(tExcept, "Problemas na confimação da transação no banco de dados");
			System.exit(9);
		}
	}
	...
	public static void desfazerTransacao() {
		try {
			...
			ExceptionUtil.mostrarErro(tExcept, "Problemas para desfazer a tansação no banco de dados");
			System.exit(9);
		}
	}
}
